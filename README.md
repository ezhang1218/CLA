...
Contrastive Liquid Association (CLA) is a contrastive dimension reduction (CDR) method for high-dimensional datasets with **multiple or continuous treatments**. Unlike standard CDR, which is limited to binary case-control comparison, CLA identifies the latent direction of variation that changes systematically with treatment intensity relative to the control group. 
